...
import { Button, List, Popover, Tooltip, Typography } from 'antd';
...
const NotificationModal = ({
	...
	const handleMarkAsRead = useCallback(
		(item?: GetMyNotificationResponse) => {
			if (!item?.read) {
			...
			if (item?.type === NotificationType.InviteJoinFolder) {
				if (
					item?.read &&
					(item.payload.status === InviteJoinFolderPayloadStatus.Expired ||
				...
				if (
					item?.payload.status === InviteJoinFolderPayloadStatus.Accepted ||
					item?.payload.status === InviteJoinFolderPayloadStatus.Active
				) {
					navigate({
						to: '/folders/$folderId',
