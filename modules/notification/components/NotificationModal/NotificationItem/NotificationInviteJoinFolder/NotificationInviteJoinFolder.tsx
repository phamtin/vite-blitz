          ...
          message.success({
            content: (
              <span>
                ...
                <span
                  onClick={() => navigate({ to: "/folders/$folderId", params: { folderId: item.payload.folderId } })}
  ...
  const invitationStatus = (item: NotificationInviteJoinFolder) => {
    switch (item.payload.status) {
      case InviteJoinFolderPayloadStatus.Expired:
        return <Tag color="orange" className={styles.inviteJoinFolderStatus}>This invite has expired. </Tag>

      case InviteJoinFolderPayloadStatus.Active:
        return <Space style={{ marginTop: '20px' }}>
          <Button onClick={(e) => {
            e.stopPropagation()
      ...
      case InviteJoinFolderPayloadStatus.Accepted:
        return <Tag color="success" className={styles.inviteJoinFolderStatus}>Invitation Accepted. </Tag>

      case InviteJoinFolderPayloadStatus.Declined:
        return <Tag color="magenta" className={styles.inviteJoinFolderStatus}>Invitation Declined. </Tag>
  ...
  return (
    ...
    >
      ...
      <Flex vertical style={{ flex: 1 }}>
        <Text><Text strong>{item.payload.invitorUsername}</Text> invited you to <Text strong>{item.payload.folderName}</Text></Text>
